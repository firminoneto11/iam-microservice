...
class Database:
    ...
    _request_db_session: Optional[async_sessionmaker["AsyncSession"]]
    ...
    @property
    def request_db_session(self):
        if self._request_db_session is None:
            raise ValueError("'_request_db_session' is None. Can not proceed")
        return self._request_db_session
    ...
    async def connect(
        ...
        self._request_db_session = async_sessionmaker(
    ...
    async def begin_session(self):
        ...
        async with self.request_db_session() as session:
